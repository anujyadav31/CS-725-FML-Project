...
class TaxiFarePrediction :
  ...
  def train(self, X_train, y_train, X_eval, y_eval, num_epochs, batch_size, optimizer, optimizer_params):
    ...
    for epoch in range(num_epochs):
        ...
        train_rmse_val = np.sqrt(train_loss)  # RMSE for training
        ...
        print(f"Epoch {epoch+1}/{num_epochs}, Train Loss: {train_loss:.4f}, Train RMSE: {train_rmse_val:.4f}, Train R²: {r2_score_train:.4f}")
        train_losses.append(train_loss)

        # Compute test loss and RMSE
        test_preds = self.forward(X_eval)
        test_loss = np.mean((y_eval - test_preds) ** 2)
        test_rmse_val = np.sqrt(test_loss)  # RMSE for testing
        ...
        # Log testing metrics
        print(f"Epoch {epoch+1}/{num_epochs}, Test Loss: {test_loss:.4f}, Test RMSE: {test_rmse_val:.4f}, Test R²: {r2_score_test:.4f}")
  ...
  def plot_metrics(self, train_losses, test_losses, train_rmse, test_rmse):
...
train_losses, test_losses , train_rmse, test_rmse = model.train(X_train, y_train, X_test, y_test,num_epochs, batch_size, optimizer, optimizer_params)
